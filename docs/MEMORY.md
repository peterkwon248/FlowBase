...
**2026-05-24 진행상태**: 6 액티비티 모드 + 일관성/깊이 + P1·P2 폴리시 19건 + **EventStore 통합 아키텍처** 완료. 직전 #5~#9 + status pill fix + 이번 큰 통합 commit `49a1767`(컬럼 리사이즈 · 레이아웃 spill fix · pill 일괄 · 스마트 메모리 · EventStore A·C·D·E·F1). **사용자 인사이트로 도출**: "스마트 메모리 = 자동 history" → 4 곳 분산 데이터를 single source로 통합 + Workspace > History 신규 surface + Detail bar Activity 탭 + AI panel derive.

**다음**: 토론 backlog 7 항목 (NEXT-ACTION 신규 섹션) — A 그룹 즉시(스마트 메모리 Phase 1B text autocomplete · Sheet groupBy + 다중 sort · Library 자산 개별 적용 UI) · B 그룹 큰 변화(EventStore Phase F2/B · Event/Comment/Thread/Snooze - W11 후) · C 그룹 부차. **AI_CLASSIFY 자동 실행은 사용자 명시 요청 시에만** (메모리 룰). B4(Library "Use in table") 후순위 유지.
