...
- **Phase 0 (Plan)**: ✅ 완료
- **Phase 1 (Beta)**: 🟢 거의 완성 — FlowBase V2. 6 액티비티 모드 + 깊이 일괄 + 폴리시 19건 + **EventStore 통합 인프라**(2026-05-24). **신규**: 컬럼 리사이즈(drag+autofit) · 레이아웃 spill fix(min-w-0 LOCK) · pill nowrap 일괄 11곳 · 스마트 메모리 Phase 1(workspaceMemory + Cell editor Recent) · EventStore Phase A·C·D·E·F1(통합 timeline + Workspace History + Detail Activity + AI panel derive + Promote bridge). **남은 폴리시**: AI_CLASSIFY 자동 실행(사용자 명시 룰) · 토론 backlog 7 항목 · Members enforcement 잔여 · 우선순위 낮음 다수.
...
**2026-05-25 진행상태**: 깊은 토론 backlog A·B 그룹 거의 완료 + Filter multi-condition 인프라 + mutation enforcement 완성. A2(Sheet groupBy+다중 sort) · Phase 1B(text autocomplete) · A3(Library OptionList apply + AddColumn 자동 link) · B1-3(promote bridge toast) · B3 F2(OptionList sync) · B1-1/B1-2(recentFilters/Sorts) · negation operators · multi-condition AND array · Wiki diff/version + 잔여 폴리시(Data Import skip · Theme accent · firedKeys ref sync · UI viewer disable · chart 터치 · Schema pinch · Members "Switch to" · mutation enforcement 24개) 모두 완료. **신규 backlog**: 상용화 차원 Import/Export 5 phase + Slack/GitHub 식 협업 layer(Snapshots + Comments).

**다음**: **Snapshots 모델** (GitHub 식, 선행 조건 ❌, 즉시 가능) · **Im-1/Im-2** (CSV·xlsx 양방향) · 협업 layer는 Phase 2 W11 후. **AI_CLASSIFY 자동 실행은 사용자 명시 요청 시에만** (메모리 룰). B4(Library "Use in table") 후순위 유지.
