...
- **Phase 0 (Plan)**: ✅ 완료
- **Phase 1 (Beta)**: 🟢 데이터 보드 자체 거의 완성 — FlowBase V2. **신규 (2026-05-25, 14 phase 통합 — multi-select 5 commits `a4e36a5`~`b6e6e10` + 13 phase 단일 commit `2c04c39`)**: Multi-select column type · Filter +Add condition UI · Dashboard 완성(D1-5/F1-2/G1/G5/G7-A/G7-B) · Phase A code-only domain fit(A1-5: domain-infer/value-format/insights/outlier) · AI 확장(B1-4/G6-1-2: 6 routes) · G3 NEXT-ACTION 잔여 · G4 안정성(auto-backup/BOM/outlier dot) · P 폴리시(onboarding/단축키 dialog/pill). **상용화 ~34%** — 백엔드/인증/협업/결제/마케팅/모바일 ❌ → M1~M7 마일스톤 9-14주 추정.
...
**2026-05-25 진행상태**: 핵심 backlog 거의 다 비웠음 — Snapshots ✅ · Im-1/2/3 ✅ · 실용성 3대 (한계가드/virtual/worker) ✅ · UI viewer disable 13 surface ✅ · Filter Match+Exclude ✅ · 보안 ✅ · Theme polish ✅. 단일 commit `c1b2be7` (44 파일 +3856/-198).

**다음**: **Multi-select column type** (큰 작업 — 모든 view 영향, 별 phase 3-5 commit 권장) · **xlsx alt lib 검토** (Phase 3+ 클라우드 sync 전 필수) · **Snapshots 후속** (cell-level diff · A vs B compare · 압축). **AI_CLASSIFY 자동 실행은 사용자 명시 요청 시에만** (메모리 룰). B4(Library "Use in table") 후순위 유지.
