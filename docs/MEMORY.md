...
- **Phase 0 (Plan)**: ✅ 완료
- **Phase 1 (Beta)**: 🟢 거의 완성 — FlowBase V2. 핵심 backlog 거의 다 비웠음. **신규 (2026-05-25 단일 commit `c1b2be7`)**: Snapshots 모델 (Compare) · Im-1/2/3 (CSV/MD · xlsx + worker · Notion/Airtable detect/normalize) · 실용성 핵심 3대 (Import 한계 가드 · Sheet virtual scrolling · Web Worker parsing) · Phase A 영어화 (~44개) · UI viewer disable 확장 (13 surface) · Filter Match+Exclude UI · Import Steps clickable navigation · Sheet useDeferredValue · Next.js 16.2.6 보안 · Theme amber WCAG · mutation enforcement 44개 완성 · mapStatus lib화 + tests. **남은**: Multi-select column type(별 phase) · xlsx alt lib(Phase 3+ sync 전) · postcss vuln(next 의존성) · 우선순위 낮음 polish 다수.
